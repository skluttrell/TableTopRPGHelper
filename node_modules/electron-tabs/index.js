...
class Tab extends EventEmitter {
  constructor (tabGroup, id, args) {
    ...
    this.iframeAttributes = args.iframeAttributes || {};
    this.iframeAttributes.src = args.src;
    this.tabElements = {};
    TabPrivate.initTab.bind(this)();
    TabPrivate.initiframe.bind(this)();
  ...
  activate () {
    ...
    if (activeTab) {
      ...
      activeTab.iframe.classList.remove("visible");
  ...
  close (force) {
    ...
    tabGroup.viewContainer.removeChild(this.iframe);
    let activeTab = this.tabGroup.getActiveTab();
...
const TabPrivate = {
  ...
  initiframe: function () {
    ...
    const tabiframeDidFinishLoadHandler = function (e) {
      this.emit("iframe-ready", this);
    };

    this.iframe.addEventListener("did-finish-load", tabiframeDidFinishLoadHandler.bind(this), false);

    const tabiframeDomReadyHandler = function (e) {
      // Remove this once https://github.com/electron/electron/issues/14474 is fixed
      iframe.blur();
      iframe.focus();
      this.emit("iframe-dom-ready", this);
    };

    this.iframe.addEventListener("dom-ready", tabiframeDomReadyHandler.bind(this), false);

    this.iframe.classList.add(this.tabGroup.options.viewClass);
    if (this.iframeAttributes) {
      let attrs = this.iframeAttributes;
      for (let key in attrs) {
        const attr = attrs[key];
        if (attr === false) continue;
        this.iframe.setAttribute(key, attr);
      }
    }

    this.tabGroup.viewContainer.appendChild(this.iframe);
  }
};
